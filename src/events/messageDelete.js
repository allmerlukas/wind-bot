...
module.exports = {
  ...
  async execute(message, client) {
    ...
    if (message.channel.id === cfg.partnerChannelId) {
      // It's a partner ad being deleted! Issue a strike.
      ...
      if (newStrikes >= 3) {
        ...
        blacklistGuild(message.guild.id, 'Deleted partner ads 3 times.');
        ...
        if (logChannel && logChannel.isTextBased()) {
          ...
          } catch { /* ignore if we can't send */ }
        }
      } else {
        // Strike 1 or 2: Send a warning
        if (logChannel && logChannel.isTextBased()) {
          try {
            await logChannel.send(`⚠️ **STRIKE ${newStrikes}/3:** A partner ad was just manually deleted from the Partner Channel! Auto-Wave relies on a fair economy. If this happens ${3 - newStrikes} more times, your server will be permanently blacklisted from the Auto-Wave network.`);
          } catch { /* ignore if we can't send */ }
        }
      }
    }
